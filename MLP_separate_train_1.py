...
from MLP_separate_1 import Config, CitationRecNet
...
def run(args):
    ...
    X_train1 = np.loadtxt(args.path + 'new-train-vec-content')
    X_train2 = np.loadtxt(args.path + 'new-train-vec-node')
    ...
    Y_train = []
    with open(args.path + 'new-train-preference-filter', 'r') as f:
        for line in f:
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for i in range(config.EPOCH):
            ...
            for j in range(STEPS):
                ...
                if j % 5 == 0:
                    y_pred, total_cross_entropy, acc = sess.run((model.y_pred_softmax, model.loss_metric, model.acc),
                                                                feed_dict={model.xa: X_test1, model.xb: X_test2, model.y: Y_test,
                                                                           model.dropout_keep: 1.0})

                    # if max_acc < acc:
                    #     max_acc = acc
                    #     print("/epoch_%s-batch_%s-total_cross_entropy_%s_acc_%s" % (i, j, total_cross_entropy, acc))
                    # model_path = args.saved_model + "model-acc.ckpt"
                    #     saver.save(sess, model_path)
                    #     output = open((args.path + 'output-year-acc'), 'w')
                    #     y_p = sess.run(model.y_pred_softmax, feed_dict={model.x: X_test, model.dropout_keep: 1.0})
                    #     for pred in y_p:
